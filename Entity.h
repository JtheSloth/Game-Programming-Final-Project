...
enum EntityType { PLAYER, ENEMY, HEARTS, DOOR, KEY, POTION, STAIRS};
enum Animation { IDLE = 0, FULL = 0, CLOSED = 0, FLOATING = 1, ATTACK = 1, EMPTY = 1, OPENING = 1, WALK = 2, OPENED = 2};
enum AIType { SPIKE, CROSSBOW, ARROW , SLIME, NONE};
...
private:
    ...
    //------- HEALTH -----------//
    int m_health;
    float m_time_since_hit = 1.0f;
    //health can only go down once per second
    float m_time_for_invincibility = 1.0f;
    //-------- DIRECTION--------//
    Direction m_direction = DOWN;

    //-------- AI VARIABLES -----//
    bool m_fired = false; //bool for if a crossbow has fired an arrow
    bool m_collected = false; //bool for it a potion or key has been collected by player
    
    //---- VECTORS -----//
    std::vector<Entity*> keys;
    std::vector<Entity*> potions;

    bool m_reached_stairs = false;
    bool m_got_new_item = false;
public:
    ...
    int       const get_health() const;
    AIType    const get_ai_type() const;
    EntityType const get_entity_type() const;
    Animation const get_animation_state() const;
    bool      const get_reached_stairs() const;
    bool      const get_new_item() const;
    ...
    void const set_health(int newHealth);
    ...
    // ————— AI METHODS ————— //
    void ai_activate(Entity* player, Entity* other_entities, int entity_count);
    void ai_sideCrossbow(Entity* player, Entity* other_entities, int entity_count);
    void ai_sideArrow();
    void ai_slime(Entity* player);

    //------- HEALTH METHODS ------//
    void lose_health(int dec);
    
    //------ ITEM METHODS -----//
    void gain_key(Entity* key);
    void gain_potion(Entity* potion);
    void use_potion();
